  ...
  <div>
    ...
    <form @submit.prevent="submitQuery" class="mx-auto grid grid-cols-8 gap-4 mb-6">
      <div class="col-span-2">
        <label class="text-left block text-sm leading-6 text-gray-900">Ranking Type</label>
        <select id="rankingType" v-model="selectedRankingType"
          class="p-4 border border-gray-300 w-full text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500">
          <option v-for="type in rankingTypes" :key="type">
            {{ type }}
          </option>
        </select>
      </div>
      ...
      <div class="col-span-1">
        <label for="indexingType" class="text-left block text-sm leading-6 text-gray-900">Count</label>
        <input type="text" placeholder="Count" v-model="selectedCount"
      ...
      <div class="col-span-5">
        <label for="indexingType" class="text-left block text-sm leading-6 text-gray-900">Search Query</label>
        <div class="relative">
    ...
    <div v-if="result" class="mt-4 pt-4 rounded-lg">
      ...
      <section class="bg-white dark:bg-gray-900">
        ...
        <div v-for="item in source" class="py-2 mx-auto">
          <div class="grid gap-2 grid-cols-1">
            <article
              ...
              <span class="text-xs text-gray-500">Node ID: <p class="text-sm font-normal text-gray-700">{{ item.node_id }}</p></span>
            </article>
          </div>
        </div>
      </section>
...
export default {
  data() {
    return {
      tables: [],
      rankingTypes: ["openai_rerank", "cohere_rerank"],
  ...
  methods: {
      ...
      if (activeInstance) {
        ...
        axios
          ...
          .then((response) => {
            ...
            this.selectedRankingType = this.rankingTypes.length > 0 ? this.rankingTypes[0] : null;
          })
          .catch((error) => {
    ...
    submitQuery() {
      ...
      const openaiApiKeyValue = sessionStorage.getItem(
        this.getStorageLabel("openai_embeddings")
      );
      const cohereApiKeyValue = sessionStorage.getItem(
        this.getStorageLabel("cohere_embeddings")
      );
      ...
      if (activeInstance) {
        ...
        axios
          .post("api/reco_engine", {
            ...
            reco_type: this.selectedRankingType,
            results_count: this.selectedCount,
            query: this.query,
          }, {
            headers: {
              'api-key': apiKeyValue,
              'openai-api-key': openaiApiKeyValue,
              'cohere-api-key': cohereApiKeyValue
            }
          })
          .then((response) => {
