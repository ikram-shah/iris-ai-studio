            ...
            <div class="grid gap-6 grid-cols-2">
              ...
              >
                <div class="flex items-center justify-left mx-auto">
                  <img
                    class="h-12 w-12 rounded-md mr-6"
                    :src="option.icon"
                    v-if="option.icon"
                    alt="Icon"
                  />
            ...
            <div>
              <component :is="selectedDataSourceComponent">
                ...
                <template v-else-if="selectedDataSource.value === 'airtable'">
                  <Airtable :uploaded-files="uploadedFiles" />
                </template>
                <template v-else-if="selectedDataSource.value === 'azure'">
                  <AzureStorage :uploaded-files="uploadedFiles" />
                </template>
              </component>
            </div>
          </div>
      ...
      <div id="page2" class="w-1/2 text-left">
        <section class="my-4 grid grid-cols-1 gap-y-4">
          ...
          <div>
            <label
              for="indexingType"
              class="text-left block text-sm leading-6 text-gray-900"
              >Indexing Type</label
            >
            <select
              id="indexingType"
              v-model="formData.indexing_type"
              class="mt-1 border border-gray-300 w-full text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500"
            >
              <option
                v-for="(type, index) in indexingTypes"
                :value="type.value"
                :key="index"
              >
                {{ type.label }}
          ...
          <div>
            <label for="modelName" class="text-left block text-sm leading-6 text-gray-900"
              >Model Name</label
            >
            <input
              type="text"
          ...
          <div>
            <label for="dimension" class="text-left block text-sm leading-6 text-gray-900"
              >Dimension</label
            >
            <input
              type="text"
              id="dimension"
              v-model="formData.embedding_dimension"
              placeholder="3072"
              class="mt-1 focus:ring-indigo-500 text-sm focus:border-indigo-500 block w-full shadow-sm border-gray-300 rounded-md"
            />
          </div>
